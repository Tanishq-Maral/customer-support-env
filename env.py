...
class CustomerSupportEnv:
    ...
    def _compute_reward(self, last_call: ToolCall) -> Reward:
        ...
        """
        step_reward = 0.1  # Base reward to ensure we never return exactly 0.0
        info: Dict[str, Any] = {}
        ...
        breakdown = RewardBreakdown()  # defaults: all components=0.1, total=0.4
        if self._done:
            breakdown = grade(
                task_id      = self.task_id.value,
            ...
            # grade() sets total strictly in (0, 1) via _set_total()
        ...
        step_reward = max(0.1, min(0.9, step_reward))
